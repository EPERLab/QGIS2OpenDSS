...
class QGIS2OpenDSS(object):
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('QGIS2OpenDSS', message)
    ...
    *conec (str): configuración de la conexión ("wye" para estrella o fase partida y "delta" para configuración delta).
    ...
    def CableAnalysis(self, carga, point, grafoBT, conns, toler, tipo_analisis = "BT" ):
        ...
        try:
            ...
            if index_carga != None:
                aviso = message + str(index_carga) + " no posee nodos vecinos. Verifique que se encuentre conectada."
            else:
                aviso = message + ext + "en (" + str(x1) + "," + str(y1) + ") no posee nodos vecinos. Verifique que se encuentre conectada."
            aviso = str( aviso + "\n")
        ...
        if error_carga == 1:
            if index_carga == None:
                aviso = "Verifique que  " + extra_message + ext + "en (" + str(x1) + "," + str(y1) + ") tenga el SERVICE correcto, ya que el tipo de línea " + cable_type + " no acepta estar conectada a la fase " + conns + "\n"
            else:
                aviso = "Verifique que " + extra_message + str(index_carga) + " tenga el SERVICE correcto, ya que el tipo de línea " + cable_type + " no acepta estar conectada a la fase " + conns + "\n"                
            self.mensaje_log_gral += aviso
            return 0
        return 1
...
##############################################################################################################################
    ...
    *errorLoadShape (bool): True si hubo errores, False en caso contrario.
    """
    def AssignLoadshapes( self, grafoCAR, folder_profile, foldername, circuitName, output_shpdss, Buses = False ) :
    ...
    def run(self):
        ...
        if result and (not projPath):
            ...
            if mensaje_mt != "":
                ...
                aviso = "Favor revise el archivo " + filename_mt + ", ya que la capa de MT presenta los siguientes errores que pueden ser corregidos: "
                if aviso_bus == True:
                    aviso += "\nLíneas de MT cob bus1 = bus2"
                if aviso_bus == True:
           ...
           ###  Lectura de datos y creación de grafo de LINEAS BAJA TENSION
            ...
            if aviso_busBT == True and mensaje_bt != "":
                aviso = "Favor revise el archivo " + filename_bt + " ya que habían líneas de BT con bus1 = bus2 en BT"
            ...
            if aviso_busAco == True and mensaje_aco != "":
                with open(dir_archivo_ACOlog, 'w') as archivo_errACOlog:
                    archivo_errACOlog.write( mensaje_aco )
                aviso = "Favor revise el archivo " + filename_aco +  " ya que habían acometidas con bus1 = bus2"
            ...
            if (buses_act == True and folder_profile != ""):  # revisa si hay cargas
                filename = circuitName + '_Loadshapes.dss'
                output_shpdss = open(foldername + '/' + filename, 'w')
                LoadShapesFile = True
                #Asigna los loadshapes de Buses
                grafoBuses, errorLoadShapeBuses, output_shpdss = self.AssignLoadshapes( grafoBuses, folder_profile, foldername, circuitName, output_shpdss, Buses = True )
            ...
            if (CARmvactive == True and folder_profile != ""):  # revisa si hay cargas
                if LoadShapesFile == False: #bandera que especifica si el archivo de loadshapes ya se abrió
                    filename = circuitName + '_Loadshapes.dss'
                    output_shpdss = open(foldername + '/' + filename, 'w')
                    LoadShapesFile = True
                
                #Asigna los loadshapes de MT
                grafoCAR_mv, errorLoadShapeMV, output_shpdss = self.AssignLoadshapes( grafoCAR_mv, folder_profile, foldername, circuitName, output_shpdss, Buses = False )
            endTimeLoadSahpe = time.time()
            ##########################   Loadshapesx
            self.progress.progressBar.setValue(55)
            startTimeWriting = time.time()
            ...
            if (CARactive == True and folder_profile != ""):  # revisa si hay cargas
                ...
                grafoCAR, errorLoadShape, output_shpdss = self.AssignLoadshapes( grafoCAR, folder_profile, foldername, circuitName, output_shpdss, Buses = False )
            endTimeLoadSahpe = time.time()
            ##########################   Loadshapesx
            self.progress.progressBar.setValue(55)
            startTimeWriting = time.time()
            ...
            if LoadShapesFile == True: #bandera que especifica si el archivo de loadshapes ya se abrió
                output_shpdss.close() 
            ...
            ### Escritura de VE
            ########################
            if name_layer_evs != "" and name_layer_evs != None and CARactive == True:
            ...
            if self.mensaje_log_gral != "":
                ...
                aviso = "Favor revise el archivo " + name_fileev + ", ya que hay errores en BT y/o VEs que pueden ser corregidos."
